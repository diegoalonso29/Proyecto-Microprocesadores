...
/*added functions to know where are the read and write pointers*/
int FAT_RdPtr(int file)
{
	return openedFiles[file].rdPtr;
}

int FAT_WrPtr(int file)
{
	return openedFiles[file].wrPtr;
}

/**
 * @brief Writes a float in the SD
 * @param file Name of the file
 * @param flot Float to write in SD
 * @return ID of file or -1 if not found.
 */
int FAT_SDWriteFloatFile(int file, float flot)
{
	...
	lenWrite=FAT_WriteFile(file,(uint8_t*)cad,g);
...
/**
 * @brief Jump to the next file in the SD file
 * @param file Name of the file
 * @return ID of file or -1 if not found.
 */
int FAT_SDJumpLineFile(int file)
{
	uint8_t special=10;//ASCII number for jumping line
	FAT_WriteFile(file,special,1);
	FAT_WriteFile(file,13,1);//ASCII cary return
