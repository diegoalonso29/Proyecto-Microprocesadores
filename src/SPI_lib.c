...
void SPI1_Init(void) {
	...
	GPIO_SetBits(GPIOB, GPIO_Pin_6); // Set SS line, deselect the device. Activates with low level bit.
	...
	SPI_InitStruct.SPI_Direction          = SPI_Direction_2Lines_FullDuplex;//so that we can send and the receive the data without waits
	SPI_InitStruct.SPI_Mode               = SPI_Mode_Master;//the nucleo will be the master
	SPI_InitStruct.SPI_DataSize           = SPI_DataSize_8b;//the information we send has a size of 8 bit
	SPI_InitStruct.SPI_CPOL               = SPI_CPOL_Low;
	SPI_InitStruct.SPI_CPHA               = SPI_CPHA_1Edge;
	SPI_InitStruct.SPI_NSS                = SPI_NSS_Soft; // software chip select
	SPI_InitStruct.SPI_BaudRatePrescaler  = SPI_BaudRatePrescaler_256;
	SPI_InitStruct.SPI_FirstBit           = SPI_FirstBit_MSB;//the bit will arrive/go in MSB..LSB order
